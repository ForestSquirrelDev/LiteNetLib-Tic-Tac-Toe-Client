...
namespace Core.Systems {
    public class PlayerInputSystem : SystemBase {
        ...
        protected override void OnUpdate(float delta) {
            ...
            SendInputMessage(_outgoingPacketsPipe, _context, x, y);
        }
        ...
        private async void SendInputMessage(OutgoingPacketsPipe outgoingPacketsPipe, SystemsContext context, int x, int y) {
            ...
            var result = await outgoingPacketsPipe.SendAndWaitForResponse(server, inputMessage, MessageType.InputResponseMessage, 2);
            if (!result.success) {
